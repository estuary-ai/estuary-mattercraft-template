...
export class SampleCharacterAnimator extends Behavior<Component> {
	...
	private _onCharacterAction(action: CharacterAction): void {
		switch (action.name) {
			...
			default:
				// This sample rig is procedural and has only four states, so it
				// can only express these two actions. Any other action you
				// define on your character arrives here. Add a case for it,
				// rather than letting it disappear silently.
				console.log(
					`SampleCharacterAnimator: No motion for action "${action.name}", ignoring`,
					action.params
				);
				break;
		}
